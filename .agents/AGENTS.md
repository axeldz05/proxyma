...
[`tests/e2e/README.md`](../tests/e2e/README.md) is the testing contract, current 28-executable/27-stable case matrix, profile map, and README decision-gate ledger.
...
* There are 28 executable cases: `full` contains 27 stable cases, 13 is quarantined, and 23 remains absent until public bidi supports multi-message input and end-to-end cancel. `functional`/`pr` include 26–28; `network` includes 25 and 29. Case 01 validates the OCR PDF download against manifest SHA-256, non-empty size, and `%PDF-` magic; 25 covers peer offline/recovery; 26 VFS peer restart; 27 unsubscribe/purge/resubscribe/open; 28 service removal across restart; 29 exact peer-to-peer VFS transfer while the sponsor is stopped.
