...
cat << 'EOF' > ./deploy/data/node-2/scripts/ocr_service.py
...
echo "📤 [HOST]: Uploading a PDF file to node-1 via HTTP..."
curl -s --cacert ./deploy/data/node-1/certs/ca.crt \
        --cert ./deploy/data/node-1/certs/node-1.crt \
        --key ./deploy/data/node-1/certs/node-1.key \
        -X POST -F "file=@test_e2e.pdf" https://localhost:8081/upload
...
FILE_HASH=$(echo "$MANIFEST_RESPONSE" | grep -o '"test_e2e.txt":{"name":"test_e2e.txt","size":[^,]*,"hash":"[^"]*"' | grep -o '"hash":"[^"]*"' | cut -d'"' -f4)
...
# Node-1 manifest to get the PDF hash
MANIFEST_RESPONSE=$(curl -s --cacert ./deploy/data/node-1/certs/ca.crt \
    --cert ./deploy/data/node-1/certs/node-1.crt \
    --key ./deploy/data/node-1/certs/node-1.key \
    https://localhost:8081/manifest)
    
PDF_HASH=$(echo "$MANIFEST_RESPONSE" | grep -o '"test_e2e.pdf":{"name":"test_e2e.pdf","size":[^,]*,"hash":"[^"]*"' | grep -o '"hash":"[^"]*"' | cut -d'"' -f4)

echo "📝 Subscribing node-2 to the PDF file..."
curl -s --cacert ./deploy/data/node-2/certs/ca.crt \
        --cert ./deploy/data/node-2/certs/node-2.crt \
        --key ./deploy/data/node-2/certs/node-2.key \
  -X POST "https://localhost:8082/subscribe?name=test_e2e.pdf"

echo "🔄 Forcing Sync on node-2 to download the PDF..."
docker compose exec node-2 ./proxyma sync

echo "⚡ Submitting OCR Task to node-2..."
curl -s --cacert ./deploy/data/node-2/certs/ca.crt \
        --cert ./deploy/data/node-2/certs/node-2.crt \
        --key ./deploy/data/node-2/certs/node-2.key \
        -X POST "https://localhost:8082/services/submit" \
        -d "{\"service\":\"ocr\", \"task_id\":\"ocr_job_1\", \"requester_node_id\":\"host-test\", \"payload\":{\"file_hash\":\"$PDF_HASH\"}}"

echo "🔍 Verifying OCR result in node-3 manifest..."
MAX_RETRIES=30
RETRY_COUNT=0
OCR_FOUND=false
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    if echo "$MANIFEST_RESPONSE" | grep -q "optimized.pdf"; then
        OCR_FOUND=true
        break
    fi
    echo "   ... OCR result not uploaded yet. Retrying ($RETRY_COUNT/$MAX_RETRIES)..."
    sleep 3
    RETRY_COUNT=$((RETRY_COUNT+1))
done

if [ "$OCR_FOUND" != true ]; then
    echo -e "${RED}❌ The OCR file was never uploaded to the VFS.${NC}"
    exit 1
fi
echo -e "${GREEN}✅ OCR Test Completed! Optimized PDF is in the network.${NC}"
