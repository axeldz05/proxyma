...
### 1. CLI — `cmd/proxyma`
* `root.go` — Cobra from `uischema.VisibleRegistry("cli")`; `resolveServiceName` / `resolvePayloadRaw`.
* `cli_actions.go` — `executeActionLocal` → Invoke + `cliEscapes`.
* `service_help.go` — help/schema; `sampleValue`; fallback `compute.LoadServicesMap`.
* `helpers.go`, `run.go`, `init.go`.

### 2. Bind — `cmd/proxyma-bind`
* `bind.go` — L1 unix + L3 **`InvokeDomainAction`** / `NormalizeActionArgs`.
...
* `local_pipelines.go` — pipelines + `NotifySchema`.
* `local_api.go` — invite, bandwidth, peers list.
* `peer_rpc.go` — **`callPeer` / `forEachPeer`** + `PeerRPC*` timeouts (fan-out + liveness).
...
* `router.go`, `holepunch.go`, `join.go`.
...
* **`services_config.go`** — SSOT `services.json`: Load/Save/Build/Upsert/Delete + **`BuildHandler`**.
...
| CLI action dispatch | `cmd/proxyma` `cliHandlers` |
